...
// Vercel serverless filesystem is read-only except /tmp — use /tmp on Vercel
// (ephemeral, perfect for a live demo; resets on cold starts). Locally use .data.
export function eng() {
  const dataDir = process.env.VERCEL
    ? path.join('/tmp', '.data-obb')
